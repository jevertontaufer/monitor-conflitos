...
async function getToken() {
  ...
  if (tokenCache.access && now < tokenCache.exp) {
...
async function fetchCountry(country, token, fromISO, toISO) {
  ...
  for (let page = 1; page <= MAX_PAGES; page++) {
    ...
    if (!res.ok) {
      throw new Error(`ACLED read ${country} p${page}: ${res.status}`);
    }
  ...
  const runners = Array.from({ length: size }, async () => {
    while (i < items.length) {
      ...
      try {
        ...
        out[c] = { events: 0, fatalities: 0, error: String(e.message || e) };
      }
    }
  });
...
async function buildIntensity() {
  ...
  return {
    ...
    source: "ACLED",
    attribution: "Dados ACLED (acleddata.com).",
    countries,
  };
}
...
server.listen(PORT, () => {
  console.log(`Monitor backend em http://localhost:${PORT}`);
  console.log(`Página: http://localhost:${PORT}/`);
  console.log(`API: http://localhost:${PORT}/intensity`);
});